...
//       // Handle 401 (Session Expired) specifically
...
//         console.error("403 Forbidden: You do not have permission to access Vehicle Refueling.")
//         alert("Permission Denied: You do not have access to read Vehicle Refueling records.")
//         setRecords([]) // Set empty so spinner stops
//         return
//       }
...
//       setRecords([])
//     } finally {
//       setIsLoading(false) // This ensures spinner always stops
//     }
//   }, [router])
...
export function RefuelingTable({
  ...
  useEffect(() => {
    fetchFrappeData()
  }, [fetchFrappeData, refreshTrigger])
                ...
                >
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      checked={selectedNames.includes(record.name)}
                      onCheckedChange={(c) =>
                        toggleRowSelection(record.name, c === true)
                      }
                    />
                  </TableCell>
                  <TableCell>{record.name}</TableCell>
