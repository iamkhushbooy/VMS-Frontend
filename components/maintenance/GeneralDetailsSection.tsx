  ...
  return (
    <div className="space-y-4 bg-slate-100/50 p-5 rounded-lg border border-slate-100">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div>
          <Label htmlFor="job_card_type" className="text-foreground mb-1">Job Cards Type</Label>
          <ReusableCombobox options={jobCardTypeOptions} value={formData.job_card_type} onValueChange={(v: string) => handleSelectChange("job_card_type", v)} placeholder="Select type" searchPlaceholder="Search..." isLoading={isBusy} />
        </div>

        <div>
          <Label htmlFor="registration_no" className="text-foreground mb-1">Registration No</Label>
        ...
        <div>
          <Label htmlFor="current_odometer_value" className="text-foreground">Current Odometer</Label>
          <Input id="current_odometer_value" name="current_odometer_value" type="number" value={formData.current_odometer_value} onChange={handleInputChange} className="mt-1 bg-input" disabled={isBusy} placeholder="Enter reading" />
        </div>
        ...
        <div>
          <Label htmlFor="status" className="text-foreground mb-1">Status</Label>
        ...
        <div>
          <Label htmlFor="date_of_initiation" className="text-foreground">Job Initiation Date</Label>
          <Input id="date_of_initiation" name="date_of_initiation" type="datetime-local" value={formData.date_of_initiation} onChange={handleInputChange} className="mt-1 bg-input" disabled={isBusy} />
        </div>

        {showTimeField &&
          <div>
            <Label htmlFor="date_of_completion" className="text-foreground">Job Completion Date</Label>
            <Input id="date_of_completion" name="date_of_completion" type="datetime-local" value={formData.date_of_completion} onChange={handleInputChange} className="mt-1 bg-input" disabled={isBusy} />
          </div>
        }
