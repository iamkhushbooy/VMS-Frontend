...
//               setEmployeeOptions(loggedUser.email)
//             } else {
...
export function MaintenanceFormModal({
  ...
  // CORRECTED: Extract 'user' from the auth store to match Utilization form
  const loggedUser = useAuthStore((s: any) => s.user)

  const [formData, setFormData] = useState({
    series: seriesOptions[0]?.name || "",
    issuer_name: loggedUser?.email || "",
    job_cards_type: "",
    current_odometer_value: "" as number | string,
    last_odometer_value: "" as number | string,
    registration_no: "",
  ...
  // Failsafe to ensure issuer_name is populated if auth loads asynchronously
  useEffect(() => {
    if (isOpen && loggedUser?.email && !formData.issuer_name) {
      setFormData((prev) => ({
        ...prev,
        issuer_name: loggedUser.email,
      }));
    }
  }, [isOpen, loggedUser?.email, formData.issuer_name]);
  ...
  useEffect(() => {
    ...
    const loadFullRecord = async (name: string) => {
      try {
        ...
        setFormData({
          series: doc.naming_series || "",
          issuer_name: doc.issuer_name || loggedUser?.email || "",
          job_cards_type: doc.job_cards_type || "",
          current_odometer_value: doc.current_odometer_value || "",
          last_odometer_value: doc.last_odometer_value || "",
          registration_no: doc.license_plate || "",
  ...
  const checkWarehouseSelection = () => {
    if (!formData.warehouse) {
      showAlert("Error", "Please select a Source Warehouse first to filter the Working Employees and Registration No.");
      return false;
    }
    return true;
  };
