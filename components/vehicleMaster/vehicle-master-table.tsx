...
import {
  ...
  PaginationPrevious,
  PaginationNext,
} from "@/components/ui/pagination" // ✅ PAGINATION IMPORTS
...
export default function VehicleMasterTable({ onAddVehicle, onSelectVehicle }: VehicleTableProps) {
  ...
  const fetchFrappeData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch all records (limit 2000) for client-side pagination
  ...
  const ITEMS_PER_PAGE = 5 // ✅ PAGINATION CONFIG
  const [currentPage, setCurrentPage] = useState(1) // ✅ PAGINATION STATE

  const totalPages = Math.ceil(filteredRecords.length / ITEMS_PER_PAGE) // ✅ CALCULATE PAGES

  const paginatedRecords = filteredRecords.slice( // ✅ SLICE DATA
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [searchTerm]) // ✅ RESET PAGE ON SEARCH
  ...
  // "Select All" affects visible paginated records
  const allVisibleSelected =
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                paginatedRecords.map((record) => ( // ✅ USE PAGINATED RECORDS
                  <TableRow
                    key={record.name}
                    className="cursor-pointer hover:bg-white/5"
                    onClick={(e) => {
