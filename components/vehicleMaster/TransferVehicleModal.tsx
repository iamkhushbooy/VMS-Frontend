...
interface TransferModalProps {
  isOpen: boolean
  onClose: () => void
  record?: any             // Used when viewing an existing Transfer Record
  prefillVehicle?: string  // Used when opening from Vehicle Master to auto-select the vehicle
}
...
export function TransferVehicleModal({ isOpen, onClose, record, prefillVehicle }: TransferModalProps) {
  ...
  const showAlert = (title: string, message: string, buttons?: AlertButton[]) => {
    setAlertState({ visible: true, title, message, buttons: buttons || [{ text: "OK", style: "cancel" }] });
  };
  ...
  useEffect(() => {
    ...
    if (record) {
      setFormData({
        ...
        toWarehouse: record.to_warehouse || "",
    ...
    const loadData = async () => {
      ...
      // Auto-fill logic when coming from Vehicle Master
      if (prefillVehicle && !record) {
        const selectedVehicle = vehicles.find(v => v.name === prefillVehicle);
        if (selectedVehicle) {
          setFormData(prev => ({
            ...prev,
            registrationNo: prefillVehicle,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const csrfRes = await fetch(getApiUrl(config.api.getCsrfToken), { credentials: "include" })
      ...
      const res = await axios.post(
        ...
        payload,
        {
          ...
          headers: { "X-Frappe-CSRF-Token": csrfToken, "Content-Type": "application/json", "Accept": "application/json" }
        }
      )
      ...
      showAlert("Transfer Failed", getErrorMessage(err), [{ text: "OK", style: "destructive" }]);
    } finally {
        ...
        <div className="p-6 border-b bg-muted/20">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">
              {isViewMode ? "Transfer Record Details" : "Transfer Vehicle"}
            </DialogTitle>
          </DialogHeader>
        </div>
            ...
            <div className="space-y-4">
              <InputGroup label="Registration no." required>
                <ReusableCombobox
                  ...
                  isLoading={isLoading}
                  disabled={isViewMode || !!prefillVehicle} // Lock if prefilled or viewing
                />
              </InputGroup>
